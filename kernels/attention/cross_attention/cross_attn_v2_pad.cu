/*
 * cross_attn_v2.cu — Cross-attention with v2 nosmem fragment-shfl pattern
 *
 * Adapts flash_attn_br16_v2 (issue #29) to cross-attention where Q (seq_q)
 * and K/V (seq_kv) have independent sequence lengths.
 *
 * smem layout (24 KB, was 48 KB in cross_attn.cu):
 *   K_tile      [Bc × D_HEAD]    FP16 = 8 KB
 *   V_tile      [Bc × D_HEAD]    FP16 = 8 KB
 *   weight_smem [Br_BLOCK × Bc]  FP16 = 8 KB
 *
 * Occupancy: 100 KB / 24 KB → 4 blocks/SM smem-permitting (was 2 at 48 KB).
 * Reg-limited at ~138 → 3 blocks/SM (was 2). +50% concurrent warps.
 *
 * Expected gain: smaller than self-attention (FA seq=1024 has 16 KV iters,
 * CLIP-77 has only ceil(77/64)=2 KV iters). Per-iter smem-traffic-reduction
 * win amortized over fewer iterations. Estimate: 15-25%.
 *
...
#define NUM_WARPS   4
#define D_HEAD      64
#define D_PAD       8                       // NEW (issue #97 padding)
#define D_STRIDE    (D_HEAD + D_PAD)        // 72 halfs = 144 bytes
#define Br_WARP     16
#define Br_BLOCK    (NUM_WARPS * Br_WARP)
#define Bc          64
#define W_PAD       8                       // NEW
#define W_STRIDE    (Bc + W_PAD)            // 72 halfs = 144 bytes
#define WMMA_M      16
#define WMMA_N      16
#define WMMA_K      16
...
// cross_attn_v2_pad: +8 padding on K_tile / V_tile / weight_smem to break
// the 128-byte smem bank period. Pattern proven on FA v2 baseline
// (Observation W follow-up, issue #97): 1.91× at seq=1024.
extern "C" __global__ __launch_bounds__(NUM_WARPS * WARP_SIZE, 3)
void cross_attn_v2_pad(
    ...
    __half *V_tile      = (__half*)(smem_raw + Bc * D_STRIDE * sizeof(__half));
    __half *weight_smem = (__half*)(smem_raw + 2 * Bc * D_STRIDE * sizeof(__half));
    ...
    // Q register cache (per-block, loaded once)
    ...
    for (int kv_base = 0; kv_base < seq_kv; kv_base += Bc) {
        ...
        // Phase A: load K, V (using seq_kv for valid-row check)
        for (int idx = global_thread; idx < Bc * D_HEAD; idx += NUM_WARPS * WARP_SIZE) {
            int kv_row    = idx / D_HEAD;
            int d_col     = idx % D_HEAD;
            int smem_off  = kv_row * D_STRIDE + d_col;     // padded destination
            int kv_global = kv_base + kv_row;
            K_tile[smem_off] = (kv_global < seq_kv) ? K_head[(size_t)kv_global * D_HEAD + d_col]
                                                  : __float2half(0.0f);
            V_tile[smem_off] = (kv_global < seq_kv) ? V_head[(size_t)kv_global * D_HEAD + d_col]
                                                  : __float2half(0.0f);
        }
        __syncthreads();
            ...
            for (int dk = 0; dk < TILES_D; dk++) {
                ...
                for (int n = 0; n < TILES_Bc; n++) {
                    ...
                    wmma::load_matrix_sync(k_frag,
                        K_tile + n * WMMA_N * D_STRIDE + dk * WMMA_K,
                        D_STRIDE);
    ...
    // Final: pv_accum / running_sum direct to global O (uses seq_q for row bounds)
    if (valid_warp) {
        ...
        bool valid_lo = (g_row_lo < seq_q);
        bool valid_hi = (g_row_hi < seq_q);
