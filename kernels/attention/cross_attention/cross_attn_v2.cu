...
void cross_attn_v2(
    ...
    for (int kv_base = 0; kv_base < seq_kv; kv_base += Bc) {
        ...
        for (int idx = global_thread; idx < Bc * D_HEAD; idx += NUM_WARPS * WARP_SIZE) {
            ...
            K_tile[idx] = (kv_global < seq_kv) ? K_head[(size_t)kv_global * D_HEAD + d_col]
                                                  : __float2half(0.0f);
            V_tile[idx] = (kv_global < seq_kv) ? V_head[(size_t)kv_global * D_HEAD + d_col]
                                                  : __float2half(0.0f);
        }
        __syncthreads();
        ...
        if (valid_warp) {
            ...
            for (int n = 0; n < TILES_D; n++) {
                ...
                for (int k = 0; k < TILES_Bc; k++) {
                    ...
                    wmma::load_matrix_sync(w_frag, warp_weight + k * WMMA_K, Bc);
