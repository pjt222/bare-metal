/*
 * flash_attn_br16_regpv_full.cu
 *   nosmem variant + bank-conflict-free padding on K/V tile and weight_smem.
 *
 * The 24 KB nosmem layout has 8 KB headroom under the 32-KB / 3-blocks-per-SM
 * threshold (100 KB SM smem / 3 blocks ~= 33 KB/block). With padding:
 *   K_tile     [Bc × STRIDE_K] FP16 = 64 × 72 × 2 = 9216 B
 *   V_tile     [Bc × STRIDE_K] FP16 = 9216 B
 *   weight_smem[Br_BLOCK × STRIDE_W] FP16 = 64 × 72 × 2 = 9216 B
 *   total: 27648 B (27.0 KB) — 3 × 27 = 81 KB ≤ 100 KB → 3 blocks/SM ✓
 *
 * Padding configuration (compile-time):
 *   -DKV_PAD_HALFS=8     pad K/V tile rows (must be 0 or multiple of 8 for WMMA)
 *   -DWEIGHT_PAD_HALFS=8 pad weight_smem rows (same constraint)
 *
 * Bank conflict status (stride 144 B = 72 halfs, gcd(36,32)=4, period=8):
 *   K/V ldmatrix.x4 col_major:    8/8 distinct banks ✓
 *   weight_smem ldmatrix.x4 row:  8/8 distinct banks ✓
 */

/* ORIGINAL HEADER FOLLOWS:
 * flash_attn_br16_v2.cu
 *   Flash Attention Br=16 WMMA, register-PV + lean state + Q cache + smem_work elimination
 *
 * Builds on flash_attn_br16_v2.cu by:
 ...
 *        flash_attn_br16_regpv_lean_qcache_nosmem.cu
 */
...
#ifndef KV_PAD_HALFS
#define KV_PAD_HALFS 8
#endif
#ifndef WEIGHT_PAD_HALFS
#define WEIGHT_PAD_HALFS 8
#endif

#define STRIDE_K (D_HEAD + KV_PAD_HALFS)
#define STRIDE_W (Bc + WEIGHT_PAD_HALFS)

extern "C" __global__ __launch_bounds__(NUM_WARPS * WARP_SIZE, 3)
void flash_attn_br16_regpv_full(
    ...
    __half *weight_smem = (__half*)(smem_raw + 2 * Bc * STRIDE_K * sizeof(__half));
    ...
    __half *warp_weight = weight_smem + warp_id * Br_WARP * STRIDE_W;
    ...
    for (int kv_base = 0; kv_base < seq_len; kv_base += Bc) {
        ...
        // Phase A: load K, V tiles to padded smem
        ...
        if (valid_warp) {
            ...
            for (int n = 0; n < TILES_Bc; n++) {
                ...
                // Write FP16 weights at row-major (row, col) positions in padded smem.
                __half *row_lo_ptr = warp_weight + row_lo * STRIDE_W + tile_col_base;
                __half *row_hi_ptr = warp_weight + row_hi * STRIDE_W + tile_col_base;
