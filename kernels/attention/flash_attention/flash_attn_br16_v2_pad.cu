/*
 * flash_attn_br16_v2_pad.cu
 *   v2 baseline + +8 padding on K_tile, V_tile, weight_smem (issue #97).
 *
 *   Per Observation W: row stride 64 halfs = 128 bytes hits the smem
 *   bank period exactly, causing 87.5% bank-conflict rate on the v2
 *   pipeline kernel. Padding stride to 72 halfs (144 bytes) eliminates
 *   the conflict (gcd(144, 128) = 16 distributes 8 rows across distinct
 *   bank groups). Pipeline saw 2.36× speedup; this is the same fix
 *   applied to the non-pipelined v2 baseline.
 *
 *   Smem cost: 24 KB → 27 KB (still fits 3 blocks/SM at 100 KB max).
 */

/*
 * Original v2 docstring follows for documentation continuity:
 *
...
#define NUM_WARPS   4
#define D_HEAD      64
#define D_PAD       8                       // NEW: pad K_tile/V_tile row stride
#define D_STRIDE    (D_HEAD + D_PAD)        // 72 halfs = 144 bytes
#define Br_WARP     16
#define Br_BLOCK    (NUM_WARPS * Br_WARP)
#define Bc          64
#define W_PAD       8                       // NEW: pad weight_smem row stride
...
void flash_attn_br16_v2_pad(
    ...
    for (int kv_base = 0; kv_base < seq_len; kv_base += Bc) {
        ...
        // Phase A: load K, V tiles — padded smem layout, source unchanged.
            ...
            for (int n = 0; n < TILES_D; n++) {
                ...
                for (int k = 0; k < TILES_Bc; k++) {
                    ...
                    wmma::load_matrix_sync(v_frag,
                        V_tile + k * WMMA_K * D_STRIDE + n * WMMA_N,
                        D_STRIDE);
