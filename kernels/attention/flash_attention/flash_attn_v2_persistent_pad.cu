/*
 * flash_attn_v2_persistent.cu
 *   v2 (nosmem fragment-shfl) on a persistent grid.
 *
 * Combines the v2 kernel's smem_work + smem_pv elimination with the
 * persistent-grid work-stealing pattern from flash_attn_persistent.cu.
 *
 * smem layout (24 KB, was 48 KB in flash_attn_persistent):
 *   K_tile      [Bc × D_HEAD]    FP16 = 8 KB
 *   V_tile      [Bc × D_HEAD]    FP16 = 8 KB
 *   weight_smem [Br_BLOCK × Bc]  FP16 = 8 KB
 *
 * Occupancy: 100 KB / 24 KB ≈ 4 blocks/SM smem-permitting.
 * Reg-limited at ~138 regs/thread × 128 × 3 = 53000 → 3 blocks (was 2).
 *
 * Persistent loop pattern: each block grabs work tiles via atomicAdd on
 * `tile_counter` until exhausted. Per-tile state (pv_accum, my_max/sum,
 * q_frag) reset at the top of each iteration.
 *
 * Build:
 *   nvcc --cubin -arch=sm_86 -O2 -o flash_v2_persistent.sm_86.cubin \
 *        flash_attn_v2_persistent.cu
 */
...
void flash_attn_v2_persistent_pad(
    ...
    while (true) {
        ...
        // Q register cache (loaded once per tile)
        ...
        for (int kv_base = 0; kv_base < seq_len; kv_base += Bc) {
            ...
            for (int idx = global_thread; idx < Bc * D_HEAD; idx += NUM_WARPS * WARP_SIZE) {
                ...
                K_tile[smem_off] = (kv_global < seq_len) ? K_head[(size_t)kv_global * D_HEAD + d_col]
                                                      : __float2half(0.0f);
                V_tile[smem_off] = (kv_global < seq_len) ? V_head[(size_t)kv_global * D_HEAD + d_col]
                                                      : __float2half(0.0f);
            }
            __syncthreads();
        ...
        // Final: pv_accum / running_sum direct to global
        if (valid_warp) {
