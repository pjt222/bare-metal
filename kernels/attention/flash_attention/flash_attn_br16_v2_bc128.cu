/*
 * flash_attn_br16_v2_bc128.cu
 *
 * REGIME-DEPENDENT VARIANT — see Observation S in docs/gpu_reflections.md.
 *
 * Hypothesis: after v2's smem reduction, Bc=128 (48 KB) fits at 2 blocks/SM,
 * giving 2x HMMA per phase. Should win on amortization.
 *
 * Result (3-run mean):
 *   seq=512  b=16 h=16: 1.888 ms vs 1.747 ms baseline = -7.4%
 *   seq=1024 b=8  h=8:  1.875 ms vs 1.757 ms baseline = -6.3%
 *   seq=2048 b=4  h=8:  3.669 ms vs 3.455 ms baseline = -5.8%
 *   seq=4096 b=2  h=8:  7.255 ms vs 7.370 ms baseline = +1.6% (WIN)
 *
 * Loses at small/medium seq where 12->8 warps occupancy hit dominates.
 * Wins at large seq where halved outer-iter count amortizes prologue/sync
 * overhead enough to overcome the occupancy hit. Crossover ~ seq=3000.
 *
 * Loses 19-23% to v2_pipeline at same 2 blocks/SM occupancy at all sizes.
 *
 * Lesson: tile-size optimizations interact with iteration count, not just
 * occupancy. Same pattern as cross-attention CLIP-77 regime split.
 *
 * Original v2 kernel docstring follows for reference.
