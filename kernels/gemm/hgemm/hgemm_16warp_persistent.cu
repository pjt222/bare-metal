/*
 * hgemm_16warp_persistent.cu — persistent-grid variant of hgemm_16warp.
 *
 * Identical inner-loop logic to hgemm_16warp.cu. Difference: launched as
 * <<<n_blocks, 512>>> with n_blocks = sm_count * 2 (one block per SM slot),
 * then each block loops over output tiles via tile_id += gridDim.x.
 *
 * Hypothesis (#86): persistent dispatch removes per-tile launch overhead
 * and keeps SMs warm across tiles. Claimed 1.15x at small problem sizes,
 * negligible at large (where launch overhead amortises).
 *
...
void hgemm_16warp_persistent(
    ...
    const int wy      = warp_id / WARPS_X;   // 0..3
    const int wx      = warp_id % WARPS_X;   // 0..3

    const int n_tile_x = (N + BN - 1) / BN;
    const int n_tile_y = (M + BM - 1) / BM;
    const int n_tiles  = n_tile_x * n_tile_y;

    // Persistent loop: each block processes multiple output tiles.
    // Using a flat tile_id keeps the assignment dense and simple.
    for (int tile_id = blockIdx.x; tile_id < n_tiles; tile_id += gridDim.x) {
      const int block_row = (tile_id / n_tile_x) * BM;
      const int block_col = (tile_id % n_tile_x) * BN;
    ...
    #undef COMPUTE_TILE
      ...
      // Sync before reusing smem buffers for next tile.
      __syncthreads();
    }   // tile loop
}
