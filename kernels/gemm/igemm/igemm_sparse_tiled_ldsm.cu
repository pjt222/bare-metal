/*
 * igemm_sparse_tiled_ldsm.cu — B-reformat with BK_STRIDE=68 for bank-conflict-free LDS.32 (issue #66)
 *
 * Layout:
 *   smem_b_temp[2][BK × BN]:    row-major for cp.async (directly from global)
 *   smem_b_reformat[2][BN × BK_STRIDE]: column-major for LDS.32 (n_col * BK_STRIDE + k_row)
 *
 * BK_STRIDE=68 chosen via R analyze_smem_layout.R: stride/4=17 (odd, coprime to 32)
 * → conflicts=0, max_share=2 for all 8 access patterns.
 *
 * SWIZZLE_B: grid-stride loop transpose temp → reformat
 * COMPUTE_TILE: inline-asm ld.shared.b32 (no PRMT)
 */
...
#define BM 128
#define BN 128
#define BK 64           // logical tile depth (algorithmic)
#define BK_STRIDE 68    // physical stride for reformat buffer (bank-conflict-free)
                        // BK_STRIDE=68 chosen via R analyze_smem_layout.R:
                        //   stride/4 = 17 (odd, coprime to 32) → conflicts=0
                        //   This works because we use inline-asm LDS.32 (not WMMA),
                        //   so alignment requirements are only 4-byte (not 16-byte).
                        //   See issue #75 for coprime-to-32 rule vs WMMA constraints.
#define BK_COMP (BK / 2)

#define STRIDE_A  (BK_COMP + 16)   // 48
...
void igemm_sparse_tiled(
    ...
    __shared__ __align__(16) int8_t smem_b_temp[2][BK * BN];
    ...
    {                                                                                 \
        ...
        if (_gr < K && _gc + 15 < N) {                                              \
            __pipeline_memcpy_async(&smem_b_temp[buf][_so],                         \
                &B[(size_t)_gr * N + _gc], 16);                                      \
        } else {                                                                     \
            for (int _b = 0; _b < 16; _b++)                                          \
                smem_b_temp[buf][_so + _b] = (_gr < K && _gc + _b < N)               \
                    ? B[(size_t)_gr * N + _gc + _b] : 0;                             \
        }                                                                            \
    }

    #define SWIZZLE_B(buf)                                                           \
    {                                                                                 \
        for (int _i = thread_id; _i < BK * BN; _i += BLOCK_SIZE) {                  \
            int _row = _i / BN;                                                      \
            int _col = _i % BN;                                                      \
            smem_b_reformat[buf][_col * BK_STRIDE + _row] = smem_b_temp[buf][_i];   \
        }                                                                            \
    }
