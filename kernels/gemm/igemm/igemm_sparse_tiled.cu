...
void igemm_sparse_tiled(
    ...
    __shared__ __align__(16) int8_t smem_a[2][BM * STRIDE_A];     // 2 x 6 KB = 12 KB
    __shared__ __align__(16) int8_t smem_b[2][BK * STRIDE_B];     // 2 x 9 KB = 18 KB
    __shared__ __align__(16) uint32_t smem_meta[2][128];          // 2 x 512 bytes = 1 KB
    // Total: 31 KB — under 50 KB cliff for 2 blocks/SM ✓
    ...
    // LOAD_META_TILE: preload metadata for 8 M-tiles × 2 K-steps × 8 gids
    // into smem_meta[128] = 512 bytes. 128 threads load 1 uint32_t each.
    #define LOAD_META_TILE(buf, tile_idx)                                              \
    {                                                                                 \
        int _gk_start = (tile_idx) * (BK / WMMA_K);                                 \
        int _flat     = thread_id;                                                   \
        if (_flat < 128) {                                                          \
            int _m_off  = _flat / 16;       /* 0..7: which M-tile in block */       \
            int _gk_off = (_flat % 16) / 8; /* 0 or 1: which K-step */              \
            int _gid    = _flat % 8;        /* 0..7 */                              \
            int _m      = block_row / WMMA_M + _m_off;                             \
            int _gk     = _gk_start + _gk_off;                                       \
            smem_meta[buf][_flat] = metadata[(size_t)_m * K_steps_total * 8          \
                                           + (size_t)_gk * 8 + _gid];               \
        }                                                                            \
    }
    //
    ...
    for (int k_step = 0; k_step < BK; k_step += WMMA_K) {                          \
        ...
        for (int wi = 0; wi < WARP_TILES_M; wi++) {                                 \
            ...
            uint32_t meta = smem_meta[buf][(wy * 2 + wi) * 16 + (_gk_idx % 2) * 8 + gid];                                                                \
                                                                                     \
    ...
    for (int tile = 0; tile < num_tiles - 1; tile++) {
        ...
        LOAD_META_TILE(next_buf, tile + 1);
        __pipeline_commit();
