...
**Store key: `(git_head, cell_id, regime)`** — grid's exact three-tuple, with `cell_id == bench_all id == grid id` and `regime == clock_label`.

> **The `cell_id == bench_all id == grid id` identity does NOT currently hold** (found in Phase 2, filed as **#160**). Six of the seven swept cells agree; `conv2d_implicit_gemm_sd64` (grid) is `conv2d_implicit_gemm` (bench_all) — same exe, same args, same `match`/`section`, different id. A `rbindlist(fill=TRUE)` rollup does not error on this: it silently yields two cell rows that read as two kernels, which is precisely what the shared key exists to prevent. Phase 5 must resolve it (alias at collect time, then adopt one name) before the two histories are merged. The Phase 2 test suite pins this as the ONE known exception, matching grid cells to bench_all configs by `(exe, args)` rather than by id, so a second divergence fails the suite. `attempt` is a non-key row column (Spine C's 4-tuple `(…, sample_idx)` was refused to preserve the resume-key invariant).
...
> **Correction (Phase 2): `fill=TRUE` does NOT align differently-named columns.** `rbindlist(fill=TRUE)` matches by name and fills absent names with NA — it never renames. A merged bench_all+grid rollup therefore carries BOTH `throttle` and `throttle_str`, each NA on half the rows; the same holds for `clock_sm_mhz` vs grid's `clock_observed_mhz` and `attempt` vs grid's `sample_idx`. Verified by rolling the real grid store through `bench_all_collect.R`. This is the same class of defect as **#160** (cell-id divergence) and belongs to the same Phase 5 reconciliation: an explicit rename map at collect time, not a hoped-for alignment. Phase 2 leaves it alone because the collector only globs `results/bench_all/`, where the divergence cannot arise.
...
2. ✅ **Phase 2 — GPU-free convergence logic — DONE 2026-07-29.** Unified spec (`bench_all.yml` gained `defaults`/`clocks`/per-kernel `regimes`, reproducing all 7 grid cells), taxonomy×regime gating with the explicit `[native]` default (`effective_regimes`/`plan_cells`/`plan_regimes` in `bench_all.R`), the #152 store columns (`regime`, `clock_target_mhz`, `band_lo`/`band_hi`, `measurable`, `verified`, `clock_mem_mhz`), and `scripts/bench/bench_all_collect.R`. Verified GPU-free: `--list` plans 71 cells over 6 regimes (49 native + 22 locked); **185 testthat assertions** pass; the collector rolls the existing pre-#152 run up, and derives regimes correctly from a foreign (grid) store too. `bench_all.R` still MEASURES the native regime only — applying a lock is Phase 3's orchestrator. Surfaced **#160**.
   - An adversarial verify pass (mutation-testing the suite against 14 deliberate breakages) caught six real defects in the first cut, all fixed before commit: `--min-valid` was killed corpus-wide by folding `defaults$n_samples` into every config; a locked cell with a missing/zero/negative/non-numeric `band_mhz` silently ran with **no** band gate; `measure_config` re-derived the band independently of the planner (now one `cell_clock_band`); the collector's medians dropped `git_head` from its own declared key (blending commits) and backfilled `regime = "native"` over rows whose `clock_target_mhz` said otherwise; `igemm_sparse_4096` silently vanished from `make bench-all` (it omits `native`) and `--only` on it was a no-op exiting 0. The assertion count *fell* 233 → 185 on purpose: 71 of the old assertions were in a loop that provably could not fail.
   - Known gaps, deliberately left: `warmup` is carried in the spec but not yet consumed (bench_all has no warmup phase; Phase 3/4 owns it); `clocks` is validated but has no expansion mechanism, so a kernel opting in must repeat the list; `summary.md` still hardcodes "Native-boost run." and does not sub-group by regime. *(was: parallel with Phase 1)*
