#!/usr/bin/env Rscript
# bench_imma_s02.R -- A/B benchmark of S08->S04 IMMA hand-tunes (#96).
#
# For each target IGEMM kernel:
#   1. Bench original cubin
#   2. Swap in the *.imma_s02.sm_86.cubin variant
#   3. Re-bench
#   4. Restore original
#
...
for (case in cases) {
    ...
    patched   <- sub("\\.sm_86\\.cubin$", ".imma_s02.sm_86.cubin", cubin)
