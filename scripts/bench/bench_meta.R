...
#
# GPU + host state capture for benchmark runs. Wraps nvidia-smi and
# /proc readers into a structured snapshot, so bench_regress.R (and
# any other harness) can attach measurement-time metadata to each
# result and refuse to compare runs at incompatible system states.
#
# Designed to be sourced, not run as a CLI:
#   source("scripts/bench/bench_meta.R")
#   pre  <- capture_gpu_state()
#   ... run bench ...
#   post <- capture_gpu_state()
#   summary <- summarise_meta(pre, post)
#
# Sources nothing else; safe to call from any harness.
#
# Captures GPU and host state around bench runs so bench_regress.R
# can skip unfair measurements (thermal throttle, power cap, etc.).

# WSL CUDA passthrough: nvidia-smi needs libnvidia-ml.so from
# /usr/lib/wsl/lib. R subprocess inherits a stripped LD_LIBRARY_PATH
# that often misses it. Same guard as scripts/bench/bench_regress.R.
...
# Decode a hex-string throttle bitmask into a character vector of
# active reason names. Empty vector means "no throttle".
...
# Snapshot GPU + host state at the current instant. Cheap (~50ms for
# nvidia-smi spawn). Safe to call before AND after each bench launch.
#
# Returns a list with shape:
#   $gpu   - $clock_sm, $clock_mem, $temp_c, $power_w, $pstate,
#            $throttle_hex, $throttle (decoded character vec),
#            $util_gpu, $util_mem
#   $host  - $loadavg, $ac_state, $gpu_mode
#   $iso_time - ISO 8601 timestamp
#
# Returns NULL if nvidia-smi fails (no GPU / not in PATH); callers
# should treat that as "no metadata available, run anyway".
...
# Decide whether a measurement is comparable to a baseline. Inputs:
#
#   pre, post  - capture_gpu_state() snapshots before / after the bench
#   valid_when - optional list with any of:
#                  $require_no_throttle  : logical (default TRUE)
#                  $allow_throttle       : character vec of names that
#                                          ARE OK (e.g. "GpuIdle")
#                  $min_clock_sm         : numeric MHz floor
#                  $max_temp_c           : numeric ceiling
#                  $require_ac           : logical (laptop)
#
# Returns list(ok = TRUE/FALSE, reasons = c("..."), summary = "...").
# If pre or post is NULL (no GPU detected), returns ok=NA.
...
# Pretty one-line summary for terminal output.
